...
st.markdown("""
    ...
    .agreement-box { border: 2px solid #b8860b; padding: 40px; background-color: #fcfcf4; border-radius: 10px; font-family: 'Inter', sans-serif; color: #333; line-height: 1.8; box-shadow: 0 4px 10px rgba(0,0,0,0.05); }
    ...
    .stMetric { background-color: #FFFFFF; padding: 20px; border-radius: 10px; border: 1px solid #E5E8E8; box-shadow: 0 2px 5px rgba(0,0,0,0.02); }
...
# Live Dashboard Counters for Sidebar Display
conn = get_db_connection()
...
# Standardized HTML Generator Functions
def generate_agreement_html(loan_row, party_row):
    ...
    if loan_row['gold_image_base64']:
        ...
        image_html_tag = "<p style='color:grey; font-style:italic; text-align:center;'>ചിത്രം ലഭ്യമാക്കിയിട്ടില്ല (No photo uploaded)</p>"
    ...
    <div class="agreement-box">
        ...
        <ul>
            ...
            <li><b>തൊഴിൽ / യോഗ്യത:</b> {party_row['occupation'] or 'N/A'} ({party_row['qualification'] or 'N/A'})</li>
        </ul>

        <h3>💰 2. വായ്പയുടെ വിവരങ്ങൾ (Loan Details)</h3>
        <table class="agreement-table">
            <tr><th>വിവരണം (Description)</th><th>തുക / നിരക്ക് (Amount / Rate)</th></tr>
            <tr><td><b>അനുവദിച്ച വായ്പ തുക (Principal / Disbursement)</b></td><td><b>₹{loan_row['principal']:,.2f}</b></td></tr>
        ...
        <h3>💎 3. പണയ വസ്തുക്കളുടെ വിവരങ്ങൾ (Gold Details)</h3>
        <ul>
            ...
            <li><b>വിലയിരുത്തിയ മൂല്യം (Appraised Value):</b> ₹{loan_row['appraised_value']:,.2f}</li>
    ...
    <div class="printable-ledger receipt-box">
        ...
        <table class="printable-table">
            ...
            <tbody>
                ...
                <tr style="font-weight:bold; background-color: #f5f5f5;"><td colspan="2" style="text-align:right;">ആകെ ലഭിച്ച തുക (Total Fees):</td><td>₹{total_fees:,.2f}</td></tr>
            </tbody>
        </table>
        <br>
        ...
        <table style="width:100%; margin-top:20px; font-size:14px; text-align:center;">
            <tr><td>____________________<br>ക്യാഷർ (Cashier)</td><td>____________________<br>കസ്റ്റമർ ഒപ്പ് (Customer)</td></tr>
        </table>
    </div>
    """
...
st.sidebar.markdown("---")
st.sidebar.markdown("### 📊 System Indicators")
st.sidebar.write(f"👤 Total Registered Parties: **{count_parties}**")
st.sidebar.write(f"💰 Active Loan Accounts: **{count_loans}**")
st.sidebar.write(f"💍 Total Vault Envelopes: **{count_gold}**")
st.sidebar.write(f"📝 Total Ledger Entries: **{count_tx}**")

st.markdown('<div class="main-title">💎 AuraLoan Core ERP</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">Enterprise Gold Loan Management Software System Node</div>', unsafe_allow_html=True)
...
if choice == "🏠 Dashboard":
    ...
    col2.metric("Total Disbursed Volume (Principal)", f"₹{total_disbursed:,.2f}")
    ...
    df_dashboard = pd.read_sql_query("""
        SELECT l.id as 'Loan ID', p.name as 'Customer Name', l.principal as 'Principal/Disbursement (₹)', 
...
# ==========================================
    ...
    if parties_df.empty:
        st.info("No customer data available inside system buffers.")
    else:
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        ...
        with st.form("edit_party_form_main"):
            ...
            if st.form_submit_button("Save All Profile Changes"):
                ...
                conn.commit()
                st.success("Customer profile database changes saved successfully.")
                st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    conn.close()
...
# ==========================================
    ...
    # 💸 SUB-MODULE 1: LOAN DISBURSEMENT ENGINE (HTML CLUTTER REMOVED)
        ...
        if not parties:
            ...
            with st.form("disbursement_calculator_form"):
                ...
                with col1:
                    ...
                    st.markdown("---")
                    st.markdown("📷 **Attach Gold Photo Attachment**")
                    ...
                    if photo_source == "Upload File":
                        ...
                        gold_photo_data = st.camera_input("Capture Live Photo Thumbnail")
                    
                with col2:
                    st.markdown("#### 📊 Simple Flat Matrix Configuration")
                    max_eligible = appraised_val * 0.75
                    st.info(f"Regulatory 75% LTV Cap Limit Value: **₹{max_eligible:,.2f}**")
                    
                    # Core Sync Pattern Configuration Rules mapping values
                    ...
                    st.markdown("---")
                    st.markdown("#### 🏷️ ആഡ്ഓൺ ചാർജ്ജുകൾ (Service Fees Ledger)")
                    ...
                    # CORE CALCULATION ASSIGNMENTS SYNCHRONIZED
                ...
                if st.form_submit_button("Approve & Disburse Capital Allocation"):
                    if principal > max_eligible:
                        st.error("Requested capital allocation violates the regulatory 75% LTV threshold cap.")
                    elif principal <= 0 or net_wt <= 0:
                        st.error("Invalid input boundary values inside data matrices.")
                    else:
                        ...
                        conn.commit()
                        st.success(f"🎉 Loan Account #{loan_id} created successfully! You can access the documents in 'Loan Agreement (Malayalam)' module.")
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    # 📊 SUB-MODULE 2: PARTY LEDGER ACCOUNTANT
        ...
        if not active_loans:
            st.info("No active open-balance files currently processing.")
        else:
            loan_options = {al['id']: f"Loan Record #{al['id']} - Holder: {al['name']} (₹{al['principal']})" for al in active_loans}
            selected_loan = st.selectbox("Select Target Portfolio File for Accounting Action", list(loan_options.keys()), format_func=lambda x: loan_options[x])
            
            tab_post, tab_view, tab_print = st.tabs(["💳 Record Repayment Entry", "📑 View Live Statement Balance", "🖨️ Generate Printable Document Sheet"])
                ...
                with st.form("repayment_ledger_post_form"):
                    ...
                    if st.form_submit_button("Post Transaction Entry"):
                        if repay_amt > 0:
                            ...
                            conn.commit()
                            st.success("Transaction entry successfully posted against portfolio references.")
                            st.rerun()
                st.markdown('</div>', unsafe_allow_html=True)
            ...
            with tab_print:
                ...
                st.download_button(label="📥 ഡൗൺലോഡ് ലെഡ്ജർ (Download HTML Ledger Sheet)", data=printable_html, file_name=f"Ledger_Loan_{selected_loan}.html", mime="text/html")
    conn.close()
...
# ==========================================
    ...
    if not loans_cursor:
        st.info("No vaulted items currently logged inside secure vault registers.")
    else:
        for row in loans_cursor:
            ...
            with col_text:
                ...
                st.write(f"👤 **ഉടമസ്ഥൻ (Borrower Profile Name):** {row['pledger_name']}")
                st.write(f"📝 **ആഭരണ വിവരണം:** {row['gold_description']}")
                st.write(f"🔢 **ആകെ എണ്ണം / തൂക്കം:** {row['items_count']} Nos / {row['net_weight']} ഗ്രാം")
                st.write(f"🔒 **വോൾട്ട് സൂചിക പെട്ടി ഐഡി (Vault Storage Tag ID):** `{row['vault_id']}`")
            with col_img:
                if row['gold_image_base64']:
                    img_bytes = base64.b64decode(row['gold_image_base64'])
                    st.image(img_bytes, caption="Pledged Security Asset Photo", width=180)
            st.markdown('</div>', unsafe_allow_html=True)
    conn.close()

# ==========================================
# MODULE: LOAN AGREEMENT MALAYALAM (DEDICATED VISUAL ACCESS)
# ==========================================
    ...
    if not available_contracts:
        st.info("No records to generate contract documentation payloads for.")
    else:
        contract_options = {ac['id']: f"Loan Account File Line #{ac['id']} - Account Owner: {ac['name']}" for ac in available_contracts}
...
# ==========================================
    ...
    if not active_loans:
        st.info("No active loan records tracking amortization maps right now.")
    else:
        loan_options = {al['id']: f"Loan Reference Line #{al['id']} (Holder: {al['name']})" for al in active_loans}
        selected_sched = st.selectbox("Select Target Loan Matrix File", list(loan_options.keys()), format_func=lambda x: loan_options[x])
        ...
        for index in range(1, target_l['duration_months'] + 1):
            ...
            schedule_list.append({
                ...
                "Payment Amount Vector (₹)": f"₹{target_l['emi']:,.2f}",
                "Remaining Balance Pool Post-Clearance (₹)": f"₹{current_rem:,.2f}"
            })
        ...
        <div class="printable-ledger">
            ...
            <table style="width:100%; margin-bottom:20px; font-size:14px;">
                ...
                <tr><td><b>ഫോൺ (Mobile):</b> {target_l['mobile']}</td><td><b>അസ്സൽ വായ്പ (Principal Amount):</b> ₹{target_l['principal']:,.2f}</td></tr>
...
# ==========================================
    ...
    st.header("💾 Core Database Infrastructure Maintenance Engine")
    conn = get_db_connection()
    
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("#### Export Registries Matrix")
        df_p = pd.read_sql_query("SELECT * FROM parties", conn)
        st.download_button("Download Customers CSV Log", data=df_p.to_csv(index=False).encode('utf-8'), file_name="parties_export.csv", mime="text/csv")
        
    with col2:
        st.markdown("#### Export Active Portfolio Ledger Tables")
        df_l = pd.read_sql_query("SELECT * FROM loans", conn)
        st.download_button("Download Loans CSV Log", data=df_l.to_csv(index=False).encode('utf-8'), file_name="loans_export.csv", mime="text/csv")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    st.download_button(
        label="📥 Download Bare-Metal Snapshot Storage (.DB Database Binary)",
