...
def generate_agreement_html(loan_row, party_row):
    ...
    if loan_row['gold_image_base64']:
        ...
        <div style="margin-top:20px; margin-bottom:20px; text-align:center;">
            ...
            <img src="data:image/jpeg;base64,{loan_row['gold_image_base64']}" style="max-width:320px; max-height:240px; border:3px solid #b8860b; border-radius:8px; box-shadow: 2px 2px 5px rgba(0,0,0,0.1);"/>
        </div>
        """
    else:
    ...
    <div class="agreement-box">
        ...
        <p><b>1. സ്വർണ്ണ ഉടമസ്ഥാവകാശ പ്രഖ്യാപനം:</b> പണയപ്പെടുത്തുന്നതിനായി ഹാജരാക്കിയിട്ടുള്ള സ്വർണ്ണാഭരണങ്ങൾ പൂർണ്ണമായും വായ്പക്കാരന്റെ സ്വന്തം ഉടമസ്ഥതയിലുള്ളതാണെന്ന് ഇതിനാൽ സാക്ഷ്യപ്പെടുത്തുന്നു.</p>
        <p><b>2. തിരിച്ചടവ് ബാധ്യതകൾ:</b> വായ്പക്കാരൻ മുകളിൽ ഒപ്പിട്ടിട്ടുള്ള നിബന്ധനകൾ പ്രകാരം എല്ലാ മാസവും കൃത്യസമയത്ത് പ്രതിമാസ തവണകളോ (EMI) അല്ലെങ്കിൽ പലിശയോ അടച്ചുതീർക്കാൻ ബാധ്യസ്ഥനാണ്.</p>
        <p><b>3. വീഴ്ചയും ലേലം ചെയ്യാനുള്ള അവകാശവും:</b> വായ്പക്കാരൻ തിരിച്ചടവിൽ വീഴ്ച വരുത്തിയാൽ, പണയം വെച്ചിരിക്കുന്ന സ്വർണ്ണം നിയമാനുസൃതമായി പരസ്യ ലേലത്തിൽ (Public Auction) വിൽക്കാനുള്ള പൂർണ്ണ അധികാരം സ്ഥാപനത്തിനുണ്ടായിരിക്കുന്നതാണ്.</p>
...
# ==========================================
    ...
    # 💸 SUB-MODULE 1: LOAN DISBURSEMENT & INSTANT AGREEMENT FORM WITH PHOTO
    if sub_choice == "💸 Loan Disbursement":
        ...
        if not parties:
            ...
            with st.form("disbursement_calculator_form"):
                ...
                if st.form_submit_button("Finalize and Disburse Capital Allocation"):
                    if principal > max_eligible:
                        ...
                        cursor.execute("INSERT INTO ledger (loan_id, transaction_type, amount, transaction_date) VALUES (?, 'Disbursement', ?, ?)", (loan_id, net_disbursed, today_str))
            ...
            # Display Instant Printable Form directly on creation finish
            if 'active_contract_loan_id' in st.session_state:
                ...
                st.markdown("---")
                st.subheader("📄 ഇൻസ്റ്റന്റ് വായ്പാ കരാർ ഫോം (Instant Agreement Form Generated)")
                
                instant_html = generate_agreement_html(loan_row, party_row)
                st.markdown(instant_html, unsafe_allow_html=True)
                
                st.download_button(
                    label="📥 ഡൗൺലോഡ് ഇൻസ്റ്റന്റ് ഫോം (Download Agreement HTML Form)",
                    data=instant_html,
                    file_name=f"Instant_Form_Loan_{l_id}.html",
                    mime="text/html"
                )
    ...
    # 📊 SUB-MODULE 2: PARTY LEDGER ACCOUNTANT WITH PRINT OPTION
        ...
        if not active_loans:
            st.info("No active profiles open currently.")
        else:
            ...
            with tab_print:
                ...
                st.download_button(label="📥 ഡൗൺലോഡ് & പ്രിന്റ് ലെഡ്ജർ (Download HTML Statement)", data=printable_html, file_name=f"Ledger_Loan_{selected_loan}.html", mime="text/html")
    conn.close()
...
# ==========================================
# MODULE: LOAN AGREEMENT MALAYALAM (DEDICATED VISUAL SHEET)
# ==========================================
    ...
    if not available_contracts:
        st.info("No records to fetch sheets for.")
    else:
        ...
        # Pull generated HTML with layout matching constraints
...
# ==========================================
    ...
    conn = get_db_connection()
    active_loans = conn.execute("SELECT id, emi, duration_months, total_payable FROM loans WHERE status='Active'").fetchall()
